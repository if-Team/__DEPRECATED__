		...
		run : function(){
			try{
				...
				lastL = new android.widget.RelativeLayout(ctx);
 				makeB.setText("Make");
				lastB.setText("Last");
				makeB.setTextColor(android.graphics.Color.argb(100,50,50,50));
				lastB.setTextColor(android.graphics.Color.argb(100,50,50,50));
				makeB.setOnClickListener(new android.view.View.OnClickListener({
					onClick:function(v){
						try{
						 	e1 = "§d [CutMap] ";
							e2 = " 지점이 설정되지 않았습니다..";
							file = new java.io.File(android.os.Environment.getExternalStorageDirectory(),"Pocketmine/plugins/! DeBePlugins/CutMap.yml");
							if (!iG){
							...
							for(x = x1; x <= x2; x++){
								for(y = y1; y <= y2; y++){
									for(z = z1; z <= z2; z++){
										if( lB == "포함") yml += " - "+(x-cP[0])+" "+(y-cP[1])+" "+(z-cP[2])+" "+Level.getTile(x,y,z)+" "+Level.getData(x,y,z)+"\n";
										else if(x !== lP[0] && y !== lP[1] && z !== lP[2]) yml += " - "+(x-cP[0])+" "+(y-cP[1])+" "+(z-cP[2])+" "+Level.getTile(x,y,z)+" "+Level.getData(x,y,z)+"\n";
										bc++;
									}
								}
							}
				...
				lastB.setOnClickListener(new android.view.View.OnClickListener({
					onClick:function(v){
						if (!iG){
				...
				makeW.setBackgroundDrawable(new android.graphics.drawable.ColorDrawable(android.graphics.Color.TRANSPARENT));
				lastW.setBackgroundDrawable(new android.graphics.drawable.ColorDrawable(android.graphics.Color.TRANSPARENT));
				makeW.showAtLocation(ctx.getWindow().getDecorView(), android.view.Gravity.LEFT | android.view.Gravity.TOP, 100, 50);
				lastW.showAtLocation(ctx.getWindow().getDecorView(), android.view.Gravity.LEFT | android.view.Gravity.TOP, 200, 50);
			}catch(err){
				print(err);				
			}				
		}
