...
class DamageBlock extends PluginBase{
	...
	public function onEnable(){
		$this->player = [];
		$this->getServer()->getScheduler()->scheduleRepeatingTask(new CallbackTask([$this,"onTick" ]), 10);
		$this->loadYml();
	}
	...
	public function onTick(){
		...
		foreach($this->getServer()->getOnlinePlayers() as $p){
			...
			if(!$p->isSurvival() || microtime(true) - $ps[$n] < 0 || $p->hasPermission("debe.damageblock.inv")) continue;
			$bb = $p->getBoundingBox();
	...
	public function loadYml(){
		...
		$this->db = (new Config($this->getServer()->getDataPath() . "/plugins/! DeBePlugins/" . "DamageBlock.yml", Config::YAML, []))->getAll();
	}

	public function saveYml(){
		ksort($this->db);
		$db = new Config($this->getServer()->getDataPath() . "/plugins/! DeBePlugins/" . "DamageBlock.yml", Config::YAML, []);
		$db->setAll($this->db);
		$db->save();
	}
