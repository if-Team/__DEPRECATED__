...
class Fishing extends PluginBase implements Listener{
	...
	public function onCommand(CommandSender $sender, Command $cmd, $label, array $sub){
		...
		switch(strtolower($sub[0])){
			...
			case "낚시대":
				if(!isset($sub[1])){
					$r = $rm . ($ik ? "아이템 <아이템ID>": "Item(I) <ItemID>");
				}else{
					$i = Item::fromString($sub[1]);
					$i = $i->getID() . ":" . $i->getDamage();
					$set["Item"] = $i;
					$r = $mm . ($ik ? "낚시 아이템을 [$i] 로 설정했습니다.": "Fishing Item is set [$i]");
				}
			break;
			case "useitem":
			case "ui":
			...
			case "소모템":
				if(!isset($sub[1])){
					$r = $rm . ($ik ? "소모템 <아이템ID> <갯수>": $rm . "UseItem(U) <ItemID> <Amount>");
				}else{
					$i = Item::fromString($sub[1]);
					$i = $i->getID() . ":" . $i->getDamage();
					$cnt = 1;
					...
					$r = $mm . ($ik ? "낚시 소모템을 [$i]  (Count: $cnt) 로 설정했습니다.": "Fishing UseItem is set [$i] (Count: $cnt)");
				}
			break;
			case "allitem":
			case "ai":
			...
			case "전체템":
				if(!isset($sub[1])){
					$r = $rm . ($ik ? "모든템 <아이템ID> <갯수>": "AllItem(A) <ItemID> <Amount>");
				}else{
					$i = Item::fromString($sub[1]);
					$i = $i->getID() . ":" . $i->getDamage();
					$cnt = 1;
					...
					$set["Item"] = $i;
					$set["UseItem"] = ["ID" => $i,"Count" => $cnt ];
					$r = $mm . ($ik ? "낚시 관련 모든템을 [$i]  (Count: $cnt) 로 설정했습니다.": "Fishing AllItem is set [$i] (Count: $cnt)");
				}
			break;
			case "delay":
			case "d":
					...
					switch(strtolower($sub[1])){
						...
						case "추가":
							if(!isset($sub[2]) || !isset($sub[3])){
								$r = $rm . ($ik ? "물고기 추가 <아이템ID> <확률> <갯수1> <갯수2>": "Fishs(F) Add(A) <ItemID> <Petsent> <Count1> <Count2>");
							}else{
		...
		$c = false;
		if($this->set->getAll() !== $set){
			$this->set->setAll($set);
			$c = true;
		}
		if($this->fish->getAll() !== $fish){
	...
	public function onPlayerInteract(PlayerInteractEvent $event){
		...
		if($this->cool[$n] == -1){
			...
			$m .= $ik ? "쿨타임 :" . round($c * 100) / -100 . " 초": "Cool : " . round($c * 100) / -100 . " sec";
			...
			$m .= $ik ? "이곳은 물이 아닙니다.": "Thare is not water";
		}else{
			$this->fishStart($p);
			unset($m);
		}
	...
	public function fishStart($p){
		$p->sendMessage($ik ? "[낚시] 낚시를 시작했습니다. 기다려주세요.": "[Fishing] Started fishing. Please wait.");
		$time = $this->getTime();
	...
	public function fishGive($p){
		...
		$p->sendMessage($ik ? "[낚시] $i (갯수:" . $i->getCount() . ") 를 얻었습니다.": "[Fishing] You get $i (count:" . $i->getCount() . ")");
	}
