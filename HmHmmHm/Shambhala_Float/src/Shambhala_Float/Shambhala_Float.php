...
class Shambhala_Float extends PluginBase implements Listener {
	...
	public function onEnable() {
		...
		foreach ( $this->getServer ()->getLevels () as $level )
			$this->db [$level->getFolderName ()] = new ShambhalaDB ( $this->getServer ()->getDataPath () . "worlds\\" . $level->getFolderName () . "\\protects.yml", $level, $this->config_Data ["default-wall-type"] );
	...
	public function onLevelLoad(LevelLoadEvent $event) {
		...
		$this->db [$level->getFolderName ()] = new ShambhalaDB ( $this->getServer ()->getDataPath () . "worlds\\" . $level->getFolderName () . "\\protects.yml", $level );
	}
	...
	public function onCommand(CommandSender $player, Command $command, $label, Array $args) {
		...
		switch (strtolower ( $command->getName () )) {
			...
			case $this->get ( "commands-welcome" ) :
				...
				break;
			case $this->get ( "commands-shambhala" ) :
				if (! isset ( $args [0] )) {
					$this->helpPage ( $player );
					return true;
				}
				switch (strtolower ( $args [0] )) {
					case $this->get ( "commands-sh-whiteworld" ) :
						$this->whiteWorld ( $player );
						break;
					case $this->get ( "commands-sh-make" ) :
						$this->protectArea ( $player );
						break;
					case $this->get ( "commands-sh-cancel" ) :
						if (isset ( $this->make_Queue [$player->getName ()] )) {
							unset ( $this->make_Queue [$player->getName ()] );
							$this->message ( $player, $this->get ( "commands-sh-cancel-help" ) );
							return true;
						} else {
							$this->alert ( $player, $this->get ( "commands-sh-cancel-fail" ) );
							return true;
						}
						break;
					case $this->get ( "commands-sh-delete" ) :
						$this->deleteHome ( $player );
						break;
					case $this->get ( "commands-sh-protect" ) :
						$this->protect ( $player );
						break;
					case $this->get ( "commands-sh-pvp" ) :
						$this->pvp ( $player );
						break;
					case $this->get ( "commands-sh-allow" ) :
						if (isset ( $args [1] )) {
						...
						break;
					case $this->get ( "commands-sh-forbid" ) :
						if (isset ( $args [1] )) {
						...
						break;
					case $this->get ( "commands-sh-homelimit" ) :
						if (isset ( $args [1] )) {
						...
						break;
					case $this->get ( "commands-sh-economy" ) :
						$this->enableEonomy ( $player );
						break;
					case $this->get ( "commands-sh-homeprice" ) :
						if (isset ( $args [1] )) {
						...
						break;
					case $this->get ( "commands-sh-message" ) :
						$this->IhatePreventMessage ( $player );
						break;
					case $this->get ( "commands-sh-help" ) :
						if (isset ( $args [1] )) {
	...
	public function Shambhala_Float(Player $player) {
		// TODO
	}
